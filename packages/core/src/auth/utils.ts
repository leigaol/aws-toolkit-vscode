...
const switchConnections = Commands.register('_aws.auth.switchConnections', (auth: Auth | TreeNode | unknown) => {
...
Commands.register('_aws.auth.help', async () => {
    await openUrl(vscode.Uri.parse(authHelpUrl))
    telemetry.aws_help.emit()
})

Commands.register('_aws.auth.signout', () => {
...
export const addConnection = Commands.register(
    { id: '_aws.auth.addConnection', telemetryThrottleMs: false },
    async () => {
...
export const reauthCommand = Commands.register('__aws.auth.reauthenticate', async (auth: Auth, conn: Connection) => {
    try {
...
Commands.register('__aws.auth.autoConnect', () => Auth.instance.tryAutoConnect())

export const useIamCredentials = Commands.register('__aws.auth.useIamCredentials', (auth: Auth) => {
...
// Legacy commands
export const login = Commands.register('_aws.login', async () => {
    const auth = Auth.instance
...
Commands.register('_aws.logout', () => signout(Auth.instance))
Commands.register('_aws.credentials.edit', () => globals.awsContextCommands.onCommandEditCredentials())
Commands.register('_aws.credentials.profile.create', async () => {
    try {
